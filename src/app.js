  ...
  try {
    ...
    pool.query(checkQuery, async (err, results) => {
      ...
      for (const value of values) {
        pool.query(
          'INSERT INTO products (id, title, price, description, category, image, rate, count) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ',
          value,
          (err, result) => {
            if (err) {
...
app.get('/fetch-products', async (req, res) => {
  try {
    ...
    for (const value of values) {
    //await pool.query('INSERT INTO products (id, title, price, description, category, image, rate, count) VALUES (?, ?, ?, ?, ?, ?, ?,?)', value);
   await pool.query('INSERT INTO products_1(id,title,description,price,discount_percentage,rating,stock,brand,category,thumbnail,image) VALUES (?, ?, ?, ?, ?, ?, ?, ?,?,?,?)', value);
    }
    res.status(200).send('Products fetched and saved to database');
  } catch (error) {
